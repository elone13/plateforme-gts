...
@section('title', 'Modifier Abonnement - GTS Afrique')
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
                <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">Modifier l'Abonnement</h1>
                <p class="text-base lg:text-lg text-gray-600">Modifier les informations de l'abonnement</p>
            </div>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 lg:px-8 py-6 border-b border-gray-200">
                <h3 class="text-xl font-semibold text-gray-900">Modifier l'abonnement</h3>
            </div>
            
            <form action="{{ route('abonnements.update', $abonnement) }}" method="POST" class="p-6 lg:p-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <select name="client_id" id="client_id" required
                            ...
                            @foreach($clients as $client)
                                <option value="{{ $client->idclient }}" {{ old('client_id', $abonnement->client_id) == $client->idclient ? 'selected' : '' }}>
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <select name="service_id" id="service_id" required
                            ...
                            <option value="">Sélectionner un service</option>
                                 ...
                                 <option value="{{ $service->id }}" {{ old('service_id', $abonnement->service_id) == $service->id ? 'selected' : '' }}>
                                     {{ $service->nom }} - {{ $service->prix ?? 'N/A' }} FCFA/mois
                                 </option>
                             @endforeach
                        </select>
                        @error('service_id')
                    ...
                    <div>
                        ...
                        <select name="statut" id="statut" required
                            ...
                            <option value="suspendu" {{ old('statut', $abonnement->statut) === 'suspendu' ? 'selected' : '' }}>Suspendu</option>
                            <option value="résilié" {{ old('statut', $abonnement->statut) === 'résilié' ? 'selected' : '' }}>Résilié</option>
                    ...
                    <div>
                        ...
                        <input type="date" name="date_debut" id="date_debut" required
                               value="{{ old('date_debut', $abonnement->date_debut->format('Y-m-d')) }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gts-primary focus:border-transparent">
                        @error('date_debut')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="duree_mois" id="duree_mois" required min="1" max="120"
                               value="{{ old('duree_mois', $abonnement->duree_mois) }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gts-primary focus:border-transparent">
                        @error('duree_mois')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                     ...
                     <div>
                        ...
                        <input type="number" name="prix_mensuel" id="prix_mensuel" required step="1" min="0"
                               value="{{ old('prix_mensuel', $abonnement->prix_mensuel) }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gts-primary focus:border-transparent">
                        @error('prix_mensuel')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                     ...
                     <div>
                         ...
                         <div class="w-full px-4 py-2 bg-gray-50 border border-gray-300 rounded-lg text-gray-600">
                             <span id="prix_total">{{ number_format($abonnement->prix_total, 0) }}</span> FCFA
                         </div>
                     </div>
                    ...
                    <div class="lg:col-span-2">
                        <div class="flex items-center">
                            <input type="checkbox" name="renouvellement_automatique" id="renouvellement_automatique" 
                                   {{ old('renouvellement_automatique', $abonnement->renouvellement_automatique) ? 'checked' : '' }}
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea name="notes" id="notes" rows="4"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gts-primary focus:border-transparent"
                                  placeholder="Informations supplémentaires sur cet abonnement...">{{ old('notes', $abonnement->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
